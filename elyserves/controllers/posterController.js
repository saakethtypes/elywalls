...
exports.deletePoster = async (req, res, next) => {
  try {
    ...
    if (req.user.utype === 'artist') {
      let ree = await Artist.findByIdAndUpdate({_id: req.user.id},
        {$pull: {postersmade: {_id:postesr._id}}});
        console.log(ree.postersmade.length)
    }
    if (req.user.utype === 'artist') {
       ree = await Artist.findByIdAndUpdate({_id: req.user.id},
        {$pull: {admires: {_id:postesr._id}}});
        console.log(ree.postersmade.length)
    }else{
       ree = await User.findByIdAndUpdate({_id: req.user.id},
        {$pull: {admires: {_id:postesr._id}}});
        console.log(ree.postersmade.length)
    }
    ...
    fs.unlink("../elyreacts/src/assets/postersDb/"+postesr.pictureURL.split('Db\\')[1], (err) => {
      if (err) {
...
exports.getRecommends = async (req, res, next) => {
  try {
    ...
    let resultTopArtists = await Artist.find({}).sort({profileViews: -1}).select({postersmade:1}).limit(5);
